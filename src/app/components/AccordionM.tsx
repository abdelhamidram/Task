...
export default function AccordionM({size, title, paragraphe, icon, display, bord, className, ...props }:AccordionProps) {
return (
<Accordion className={twMerge("w-[480px] h-8", size== 'sm'?"h-8":size== 'md'?"h-10":size== 'lg'?"h-12":className)} {...props } type="single" collapsible> 
  <AccordionItem value="item-1" className={twMerge("px-4 py-1",display == 'active'?"border-b border-r border-l border-gray-300 bg-[#F4F4F4]":
   ...
   bord=='bottom'?"rounded-b-lg":className)} {...props }>
    <AccordionTrigger {...props } className={twMerge(display == 'active'?"border-b border-gray-300":display == 'hover'?"border-b border-blue-600":className)}>Choose an option</AccordionTrigger>
    {display !== 'disabled'?
    <AccordionContent className="w-[480px] h-[180px] py-1" {...props }>
      <div {...props } className="w-[445px] h-[164px] bg-blue-200 border border-blue-400 rounded-md flex flex-col items-start p-4 gap-4">
        {/* {icon === <HiArrowPath className="text-3xl min-h-6 min-w-6" {...props }/>? {...props}: "Icon"} */}
